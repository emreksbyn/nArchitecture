    ...
    public class AuthManager : IAuthService
    {
        ...
        public async Task<AccessToken> CreateAccessToken(User user)
        {
            IPaginate<UserOperationClaim> userOperationClaims = await _userOperationClaimRepository
                .GetListAsync(uoc => uoc.UserId == user.Id,                                                                                           include: uoc => uoc.Include(uoc => uoc.OperationClaim));

            IList<OperationClaim> operationClaims = userOperationClaims.Items
                .Select(uoc => new OperationClaim { Id = uoc.OperationClaim.Id, Name = uoc.OperationClaim.Name }).ToList();
